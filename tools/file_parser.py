...
_TEXT_EXT = {".py", ".txt", ".md", ".json", ".csv", ".log", ".yaml", ".yml", ".ini", ".cfg",
             ".html", ".css", ".js", ".ts", ".java", ".c", ".cpp", ".h", ".sh", ".sql", ".xml", ".toml"}
_PDF_EXT = {".pdf"}
_DOCX_EXT = {".docx"}
_IMAGE_EXT = {".png", ".jpg", ".jpeg", ".bmp", ".tiff"}
_NOTEBOOK_EXT = {".ipynb"}

SUPPORTED_EXT = _TEXT_EXT | _PDF_EXT | _DOCX_EXT | _IMAGE_EXT | _NOTEBOOK_EXT
...
def _parse_notebook(fp: Path) -> str:
    """解析 Jupyter Notebook(.ipynb)：按顺序提取每个 cell 的源码与 Markdown，
    输出为带 cell 标注的纯文本（忽略输出和图片，避免噪声）。"""
    import json
    try:
        nb = json.loads(fp.read_text(encoding="utf-8", errors="replace"))
    except Exception as e:  # noqa: BLE001
        return f"[解析失败] ipynb 不是合法 JSON: {type(e).__name__}: {e}"

    cells = nb.get("cells", [])
    if not cells:
        return "[提示] notebook 为空（无 cell）"
    parts = []
    code_no = 0
    for cell in cells:
        ...
        if ctype == "code":
            code_no += 1
            parts.append(f"# ===== 代码单元 [{code_no}] =====\n{src}")
        elif ctype == "markdown":
            parts.append(f"# ----- Markdown 说明 -----\n{src}")
    if not parts:
        return "[提示] notebook 无有效内容"
    return "\n\n".join(parts)
...
def _locate_tesseract():
    """自动定位 Tesseract 引擎可执行文件路径（优先配置，其次常见安装位置）。"""
    import os
    import shutil
    ...
    # 2) PATH 中查找（云端 apt 安装即在 PATH 内）
    found = shutil.which("tesseract")
    if found:
        return found
    # 3) 常见 Windows / conda 安装位置
    candidates = [
    ...
    if conda:
        candidates.append(os.path.join(conda, "Library", "bin", "tesseract.exe"))
        candidates.append(os.path.join(conda, "bin", "tesseract"))
    for c in candidates:
        if os.path.exists(c):
            return c
    return None
...
def _parse_image_ocr(fp: Path) -> str:
    try:
        ...
        return ("[图片无法识别] 当前环境未安装图片识别组件。"
                "建议：直接把图片中的报错文字复制成文本发给我，识别更准更快。")

    from config.config import CONFIG
    # 自动定位 Tesseract 引擎
    engine = _locate_tesseract()
    if engine:
        pytesseract.pytesseract.tesseract_cmd = engine
    try:
        img = Image.open(str(fp))
        # 优先中英混合，失败再退英文，避免缺中文包时直接报错
        try:
            text = pytesseract.image_to_string(img, lang=CONFIG.ocr_lang)
        except Exception:  # noqa: BLE001  语言包缺失等
            text = pytesseract.image_to_string(img)
        if not text.strip():
            return "[提示] 图片中未识别到文字（可能无文本或清晰度不足），可尝试直接粘贴报错文字。"
        return text
    except pytesseract.TesseractNotFoundError:
        return ("[图片无法识别] 未找到 Tesseract OCR 引擎（当前运行环境未安装）。"
                "建议：直接把图片里的报错文字复制成文本发给我，这样识别更准确。")
    except Exception as e:  # noqa: BLE001
...
def parse_file(path: str) -> str:
    ...
    ext = fp.suffix.lower()
    if ext in _NOTEBOOK_EXT:
        raw = _parse_notebook(fp)
