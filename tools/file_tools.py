...
# ------------------------------------------------------------
# 工具：定点替换修改代码（推荐方式，避免整文件重写）
# ------------------------------------------------------------
@tool(description="对项目内文件做定点替换修改：把指定的原始片段替换为新片段，只需提供改动部分，无需重写整个文件。修改前自动备份")
def edit_code(path: str, old_snippet: str, new_snippet: str, backup: bool = True) -> str:
    """在文件中把 old_snippet 精确替换为 new_snippet（只改必要片段）。

    用法要点：
    - old_snippet 必须与文件中的原文**完全一致**（含缩进/空格），且在文件中**唯一**；
      为保证唯一定位，可多带上下文几行。
    - 只提供需要改动的局部片段，不要把整个文件贴进来。
    - 一次只改一处；多处修改请分多次调用。

    Args:
        path: 目标文件路径（相对工作区或白名单内绝对路径）
        old_snippet: 文件中现有的、要被替换的原始片段（需与原文完全一致且唯一）
        new_snippet: 替换后的新片段
        backup: 修改前是否备份原文件为 .bak
    """
    fp = _resolve_safe(path)
    if not fp.exists():
        return f"[错误] 文件不存在: {path}"
    ...
    if old_snippet == "":
        return "[失败] old_snippet 不能为空。如需新建文件请用 write_file。"
    count = text.count(old_snippet)
    if count == 0:
        return (
            "[失败] 在文件中未找到要替换的原始片段（old_snippet）。\n"
            "请先用 read_file 确认原文，old_snippet 必须与文件内容逐字一致（含缩进和空格）。"
        )
    if count > 1:
        return (
            f"[失败] 原始片段在文件中出现了 {count} 次，无法唯一定位。\n"
            "请在 old_snippet 中多包含上下文几行，使其在文件中唯一。"
        )
    ...
    # 给出改动前后的小范围对照，便于模型与用户确认
    old_lines = old_snippet.strip("\n").splitlines()
    new_lines = new_snippet.strip("\n").splitlines()
    diff_preview = (
        "--- 原片段 ---\n" + "\n".join(f"- {l}" for l in old_lines[:15])
        + "\n--- 新片段 ---\n" + "\n".join(f"+ {l}" for l in new_lines[:15])
    )
    return f"[成功] 已修改 {fp.name} {backup_note}\n{diff_preview}"
