...
def _pick_mascot():
    ...
    import random
    mascot_dir = CONFIG.paths.assets / "mascot"
    if not mascot_dir.exists():
        return None
    imgs = sorted(mascot_dir.glob("*.png")) + sorted(mascot_dir.glob("*.jpg"))
    ...
    with mid:
        ...
        with ci:
            if st.session_state.mascot:
                st.image(st.session_state.mascot, width=130)
        with ct:
            st.markdown(
                f"<div style='font-size:20px;color:#333;line-height:1.6;padding-top:6px'>"
