...
# 云端启动时知识库可能为空，或其索引(fallback_store.tsv)与 knowledge/docs/ 下的
# 实际文档不一致（例如新增了文档但索引仍是旧快照）。这里做"自愈"：
# 比对"磁盘上的文档集合"与"知识库已记录的来源集合"，不一致就从 docs/ 重建。
# 关键词模式重建轻量、秒级、无需 torch。
@st.cache_resource(show_spinner="正在初始化知识库…")
def _ensure_knowledge_base():
    try:
        from tools.knowledge_tools import get_engine
        from tools.file_parser import SUPPORTED_EXT
        engine = get_engine()

        # 磁盘上 docs/ 目录里的文档名集合
        docs_dir = CONFIG.paths.knowledge_docs
        disk_files = set()
        if docs_dir.exists():
            disk_files = {
        ...
        # 知识库索引里已记录的来源集合
        indexed = {name for name, _ in engine.list_sources()} if engine.count() else set()

        # 空库，或磁盘文档与已入库来源不一致 → 重建
        if engine.count() == 0 or (disk_files and disk_files != indexed):
            from knowledge.init_kb import init_knowledge_base
            init_knowledge_base(reset=True)
        return get_engine().count()
    except Exception:  # noqa: BLE001  知识库初始化失败不应阻断主程序
        return 0
...
def _time_greeting() -> str:
    ...
    if 5 <= h < 11:
        ...
        period = "夜深了"
    tails = [
        "我能为你解决什么 PyTorch 或代码问题？",
        "你想问我点什么呢？",
        "今天想调试或修改哪段代码？",
        "有什么代码问题，尽管交给我吧。",
        "把你的代码或报错丢给我看看吧。",
    ]
    return f"{period}，{random.choice(tails)}"


def _pick_mascot():
    """随机挑一张吉祥物图片，返回路径（无则 None）。
    依次在 assets/mascot/、assets/ 根目录中查找；优先用英文名(rabbit*)图片，
    避免中文文件名在云端(Linux)部署时出问题。"""
    import random
    search_dirs = [CONFIG.paths.assets / "mascot", CONFIG.paths.assets]
    for d in search_dirs:
        if not d.exists():
            continue
        ascii_imgs = sorted(d.glob("rabbit*.png")) + sorted(d.glob("rabbit*.jpg"))
        if ascii_imgs:
            return str(random.choice(ascii_imgs))
...
# 问候语与吉祥物：每个会话固定一次，避免 rerun 时闪烁
if "greeting" not in st.session_state:
    st.session_state.greeting = _time_greeting()
if "mascot" not in st.session_state:
    st.session_state.mascot = _pick_mascot()
...
with st.sidebar:
    ...
    st.caption("数值越低回答越稳定严谨，越高越发散有创意")
    max_iter = st.slider("最大迭代轮数", 3, 25, CONFIG.max_iterations, 1)
    st.caption("一次任务最多思考/调用工具的轮数，越大越能处理复杂问题")
...
# ---- 输入区（chat_input 始终固定在页面底部，代码位置不影响其视觉位置）----
_chat = st.chat_input(
    "描述你的 PyTorch 代码问题，或要求按需修改代码…",
    accept_file="multiple",
    file_type=["py", "ipynb", "txt", "json", "log", "md", "csv", "yaml", "yml",
               "html", "css", "js", "ts", "java", "c", "cpp", "h", "sh", "sql",
               "png", "jpg", "jpeg"],
)
...
# ---- 空状态：居中显示问候语 + 吉祥物（无对话且未在提交时才显示；思考生成后页面不变动）----
if not conv.get("turns") and not user_text:
    st.markdown("<div style='height:12vh'></div>", unsafe_allow_html=True)
    _, mid, _ = st.columns([1, 3, 1])
    with mid:
        ci, ct = st.columns([1, 3], vertical_alignment="center")
        with ci:
            if st.session_state.mascot:
                st.image(st.session_state.mascot, width=170)
        with ct:
            st.markdown(
                f"<div style='font-size:30px;font-weight:700;color:#222;"
                f"line-height:1.5;padding-top:28px'>"
                f"{_esc(st.session_state.greeting)}</div>",
